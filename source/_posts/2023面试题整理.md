...
2. 箭头函数没有自己的arguments对象向，可以再箭头函数中使用rest参数替代arguments来访问箭头函数的参数列表
...
2. vue相对语法简单，适用于小型项目，weact更侧重于大型应用
...
4. 可以使当前url同源的任意url
5. history原理：history.pushState事件，当浏览器跳转到新的状态时会触发popstate事件 window.history的相关api
...
可以利用html5的属性data-xxx来保存图片的路径，当我们需要加载图片的时候才将data-xxx的值赋予src,就能实现突破的按需加载
...
第2次挥手：服务端收到FIN后，发送一个ACK给客户端，确认序号为收到序号+1（与SYN相同，一个FIN占用一个序号），服务端进入CLOSE_WAIT状态； **好的收到**
第3次挥手：服务端发送一个FIN，用来关闭服务端到客户端的数据传送，服务端进入LAST_ACK状态； **我数据发送完了**
第4次挥手：客户端收到FIN后，客户端t进入TIME_WAIT状态，接着发送一个ACK给Server，确认序号为收到序号+1，服务端进入CLOSED状态，完成四次挥手  **好的拜拜**
...
2. 最大区别： composition api 与 vue2的options api相比更简便和整洁
3. 生命周期钩子变了
```js
Vue2--------------vue3
beforeCreate  -> setup()
